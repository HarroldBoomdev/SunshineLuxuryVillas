...
document.addEventListener('DOMContentLoaded', () => {
  ...
  /**
   * Optional validation (kept, but NOT used for navigation now)
   * If later you want it back, call validateStep(current) before showStep(...)
   */
  function validateStep(n) {
    ...
    if (n === 1) {
      ...
      if (ref && !ref.value.trim()) ok = false;
      return ok;
    }
    return true;
  }
  ...
  // Top step nav (BYPASS validation)
  stepNavItems.forEach(btn => {
  ...
  if (firstInvalid) {
    ...
    if (stepSection) initialStep = (+stepSection.dataset.step || 1);
  } else {
    const params = new URLSearchParams(window.location.search);
    const urlStep = parseInt(params.get('step') || '1', 10);
    if (!isNaN(urlStep) && urlStep >= 1 && urlStep <= TOTAL) {
      initialStep = urlStep;
    }
  }
